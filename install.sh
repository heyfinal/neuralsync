...
# Update status - simplified without cursor positioning
update_status() {
    local task="$1"
    CURRENT_TASK="$task"
...
#!/bin/bash
"$INSTALL_DIR/venv_aider/bin/aider" "\$@"
EOF
                chmod +x "$INSTALL_DIR/bin/aider"
            fi
            DETECTED_AIS="$DETECTED_AIS aider"
            ;;
        ...
        "fabric")
            ...
            if command_exists pipx; then
                ...
                # Create wrapper script
                cat > "$INSTALL_DIR/bin/fabric" << EOF
#!/bin/bash
"$INSTALL_DIR/venv_fabric/bin/fabric" "\$@"
EOF
                chmod +x "$INSTALL_DIR/bin/fabric"
            fi
            DETECTED_AIS="$DETECTED_AIS fabric"
            ;;
        ...
        case "$1" in
            ...
            fabric)
                ...
                if command -v pipx >/dev/null 2>&1; then
                    ...
                    brew install pipx && pipx install fabric
                fi
                ;;
            ollama)
            ...
            gpt5-planner|chatgpt)
                ...
                if command -v pipx >/dev/null 2>&1; then
                    ...
                    brew install pipx && pipx install chatgpt-cli
                fi
                ;;
            *)
...
main() {
    ...
    # Show final banner without clearing screen
    echo ""
    show_banner
